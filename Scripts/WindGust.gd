...
const TrailFollow2DScript = preload("res://Scripts/TrailFollow2D.gd")
...
func update_path_gradient():
	var point_count = min(gradient.get_point_count(), pf_dict.size())
	for pcnt in range(point_count):
		var trail_color = trail_color_gradient.sample(gradient.get_offset(pcnt))
		trail_color.a *= trail_color_gradient.sample(pf_dict[pcnt].progress_ratio).a
		gradient.set_color(pcnt, trail_color)
...
func init_path_followers():
	...
	for pf_cnt in range(line_segments + 1):
		var new_pf = TrailFollow2DScript.new()
	...
	for g_cnt in range(1, line_segments):
		gradient.add_point(float(g_cnt) / float(line_segments), Color(1.0, 1.0, 1.0, 1.0))
